  ...
  return (
    <main style={{ fontFamily: "ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif", color: colors.ink, background: `linear-gradient(180deg, ${colors.soft} 0%, #fff 46%, #f7fbff 100%)` }}>
      ...
      <section style={{ position: "relative", color: colors.white, overflow: "hidden", background: colors.navy }}>
        <video autoPlay muted loop playsInline poster="/landing/hero-poster.jpg" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: .42, pointerEvents: "none" }}><source src="/landing/hero-video.mp4" type="video/mp4" /></video>
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, rgba(7,23,40,.94) 0%, rgba(7,23,40,.82) 45%, rgba(18,49,74,.68) 100%)" }} />
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 18% 20%, rgba(125,220,202,.22), transparent 30%)" }} />
        <div style={{ ...wrap, position: "relative", zIndex: 2, paddingTop: 64, paddingBottom: 72 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 28, alignItems: "center" }}>
            <div>
              ...
              <h1 style={{ margin: 0, fontSize: "clamp(44px, 7vw, 72px)", lineHeight: ".96", letterSpacing: "-.055em", textShadow: "0 8px 30px rgba(0,0,0,.28)" }}>Local window cleaning that doesn’t feel rushed.</h1>
              <p style={{ marginTop: 20, fontSize: 18, lineHeight: 1.75, color: "#dbeff4", maxWidth: 680 }}>We help homes around {areaDescription} look cleaner and brighter with detailed glass, screens, tracks, sills, hard water help, and solar panel add-ons.</p>
              <div style={{ marginTop: 22, padding: "16px 18px", borderRadius: 22, background: "rgba(7,23,40,.48)", border: "1px solid rgba(125,220,202,.30)", maxWidth: 620, backdropFilter: "blur(8px)" }}><div style={{ fontSize: 22, fontWeight: 1000 }}>Get $75 off your first full-service window cleaning.</div><div style={{ marginTop: 6, color: "#d0f2ec", fontWeight: 800 }}>Fair pricing. Careful work. No pushy upsells.</div></div>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 24 }}><a href="#quote" style={{ ...btn, background: `linear-gradient(135deg, ${colors.mint}, ${colors.sky})`, color: colors.navy }}>Get Free Quote</a><a href={phoneHref} style={{ ...btn, background: "rgba(255,255,255,.10)", color: colors.white, borderColor: "rgba(255,255,255,.28)" }}>Call Now</a><a href="/" style={{ ...btn, background: colors.white, color: colors.navy }}>Check out our website</a></div>
            </div>
            <div style={{ ...card, padding: 16, background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.22)", backdropFilter: "blur(12px)" }}><div style={{ minHeight: 360, borderRadius: 22, background: `linear-gradient(145deg, rgba(125,220,202,.24), rgba(114,200,232,.16)), linear-gradient(135deg, #fff, #eef8fb)`, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", inset: 26, borderRadius: 28, border: "2px dashed rgba(7,23,40,.18)" }} /><div style={{ position: "absolute", top: 26, left: 26, right: 26, background: colors.white, color: colors.navy, borderRadius: 20, padding: 16, boxShadow: "0 18px 40px rgba(7,23,40,.10)" }}><div style={{ fontSize: 13, fontWeight: 1000, color: colors.mintDark, textTransform: "uppercase", letterSpacing: ".08em" }}>Real job photos below</div><div style={{ marginTop: 6, fontSize: 24, fontWeight: 1000, letterSpacing: "-.03em" }}>Before / after results</div><div style={{ marginTop: 8, color: colors.muted, lineHeight: 1.6, fontSize: 14 }}>Hard water, dirt and grime, and solar panel cleaning examples.</div></div><div style={{ position: "absolute", bottom: 26, left: 26, right: 26, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><div style={{ borderRadius: 18, padding: 18, background: "rgba(7,23,40,.80)", color: colors.white, fontWeight: 900 }}>Before</div><div style={{ borderRadius: 18, padding: 18, background: `linear-gradient(135deg, ${colors.mintSoft}, ${colors.skySoft})`, color: colors.navy, fontWeight: 1000 }}>After</div></div></div></div>
          </div>
        </div>
      </section>
